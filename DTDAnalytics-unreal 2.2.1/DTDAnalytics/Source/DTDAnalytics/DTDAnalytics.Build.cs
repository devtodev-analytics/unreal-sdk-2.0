...
public class DTDAnalytics : ModuleRules
{
    public DTDAnalytics(ReadOnlyTargetRules Target) : base(Target)
    {
        ...
        if (Target.Platform == UnrealTargetPlatform.Android)
        {
            ...
            {
                AdditionalPropertiesForReceipt.Add("AndroidPlugin", Path.Combine(ModuleDirectory, "DTDAnalytics_UPL_Android_Pack_1.xml"));
            }
            else if (Target.Version.MajorVersion == 5 && Target.Version.MinorVersion == 3)
            {
                AdditionalPropertiesForReceipt.Add("AndroidPlugin", Path.Combine(ModuleDirectory, "DTDAnalytics_UPL_Android_Pack_2.xml"));
            }

            if (!IS_COPPA_ENABLED)
            {
                AdditionalPropertiesForReceipt.Add("AndroidPlugin", Path.Combine(ModuleDirectory, "DTDAnalytics_UPL_Google.xml"));
            }
        }
