...
void HelperJNI::CallStaticMethod(const ANSICHAR* className, const ANSICHAR* methodName, double argument)
{
	JNIEnv* Env = FAndroidApplication::GetJavaEnv();
	jclass jclsAnalytics = FAndroidApplication::FindJavaClass(className);
	jmethodID jmidMethod = Env->GetStaticMethodID(jclsAnalytics, methodName, "(D)V");
	Env->CallStaticVoidMethod(jclsAnalytics, jmidMethod, argument);
}
