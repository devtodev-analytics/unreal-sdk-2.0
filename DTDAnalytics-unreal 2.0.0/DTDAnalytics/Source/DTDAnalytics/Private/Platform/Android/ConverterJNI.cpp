...
jobject ConverterJNI::ConvertToJObject(const FDTDAnalyticsConfiguration& config)
{
	...
	// Set log level.
	jfieldID jfidLogLevel = Env->GetFieldID(jclsAnalyticsConfiguration, "logLevel", "Lcom/devtodev/analytics/external/DTDLogLevel;");
	jobject joLoglevel = ConverterJNI::ConvertToJavaLogLevel(config.LogLevel);
	Env->SetObjectField(joAnalyticsConfiguration, jfidLogLevel, joLoglevel);
	...
	if (config.CurrentLevel.HasValue)
	{
		...
		jfieldID jfidCurrentLevel = Env->GetFieldID(jclsAnalyticsConfiguration, "currentLevel", "Ljava/lang/Integer;");
		Env->SetObjectField(joAnalyticsConfiguration, jfidCurrentLevel, joCurrentLevel);
	...
	if (config.UserId.HasValue)
	{
		...
		jfieldID jfidUserId = Env->GetFieldID(jclsAnalyticsConfiguration, "userId", "Ljava/lang/String;");
		Env->SetObjectField(joAnalyticsConfiguration, jfidUserId, jsUserId);
		Env->DeleteLocalRef(jsUserId);
	}

	// Set tracking avaibility.
	jfieldID jfidTrackingAvailability = Env->GetFieldID(jclsAnalyticsConfiguration, "trackingAvailability", "Lcom/devtodev/analytics/external/DTDTrackingStatus;");
	jobject joTrackingStatus = ConverterJNI::ConvertToJTrackingStatus(config.TrackingAvailability);
	Env->SetObjectField(joAnalyticsConfiguration, jfidTrackingAvailability, joTrackingStatus);
	Env->DeleteLocalRef(joTrackingStatus);
...
jobject ConverterJNI::ConvertToJObject(FDTDStartProgressionEventParams value)
{
	...
	if (value.Difficulty.HasValue)
	{
		...
		jfieldID jfidDifficulty = Env->GetFieldID(jclsParams, "difficulty", "Ljava/lang/Integer;");
		Env->SetObjectField(joParams, jfidDifficulty, joDifficulty);
	...
	if (value.Source.HasValue)
	{
		...
		jfieldID jfidSource = Env->GetFieldID(jclsParams, "source", "Ljava/lang/String;");
		Env->SetObjectField(joParams, jfidSource, jsSource);
...
jobject ConverterJNI::ConvertToJObject(FDTDFinishProgressionEventParams value)
{
	...
	jfieldID jfidSuccessfulCompletion = Env->GetFieldID(jclsParams, "successfulCompletion", "Z");
	Env->SetBooleanField(joParams, jfidSuccessfulCompletion, value.SuccessfulCompletion);

	// Set duration.
	jfieldID jfidDuration = Env->GetFieldID(jclsParams, "duration", "I");
	Env->SetIntField(joParams, jfidDuration, value.Duration);

	// Set spent.
	jobject joSpent = ConverterJNI::ConvertToJMap(value.Spent);
	jfieldID jfidSpent = Env->GetFieldID(jclsParams, "spent", "Ljava/util/Map;");
	Env->SetObjectField(joParams, jfidSpent, joSpent);
	Env->DeleteLocalRef(joSpent);

	// Set earned.
	jobject joEarned = ConverterJNI::ConvertToJMap(value.Earned);
	jfieldID jfidEarned = Env->GetFieldID(jclsParams, "earned", "Ljava/util/Map;");
	Env->SetObjectField(joParams, jfidEarned, joEarned);
