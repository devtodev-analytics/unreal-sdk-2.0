...
class AndroidAnalytics : public PlatformAnalytics
{
private:
	TMap<FString, FDTDRemoteConfigValue> localConfig;
...
public:
	...
	void CoppaControlEnable() override;
	void SetInitializationCompleteCallback(const FDTDAnalyticsInitializationCompleteDelegate* callback) override;
	...
	virtual void SetTester(bool tester) override;
	...
	virtual void InitializeWithAbTest(const FString& appKey,
		...
		const FDTDRemoteConfigReceiveResultDelegate* onRemoteConfigReceive) override;
	virtual void InitializeWithConfigWithAbTest(const FString& appKey,
		const FDTDAnalyticsConfiguration& config, 
		const FDTDRemoteConfigChangeResultDelegate* onRemoteConfigChange, 
		const FDTDRemoteConfigPrepareToChangeDelegate* onRemoteConfigPrepareToChange,
		const FDTDRemoteConfigReceiveResultDelegate* onRemoteConfigReceive) override;
	virtual float GetRemoteConfigWaiting() override;
	virtual void SetRemoteConfigWaiting(float value) override;
	virtual float GetGroupDefinitionWaiting() override;
	virtual void SetGroupDefinitionWaiting(float value) override;
	virtual void SetDefaults(const FDTDRemoteConfigDefaults& defaults) override;
	virtual void ApplyConfig() override;
	virtual void ResetConfig() override;
    virtual TMap<FString, FDTDRemoteConfigValue> GetConfig() override;
	virtual FDTDRemoteConfigValue GetRemoteConfigValue(const FString& key) override;
	virtual bool HasKey(const FString& key) override;
	virtual void CacheTestExperiment() override;
};
