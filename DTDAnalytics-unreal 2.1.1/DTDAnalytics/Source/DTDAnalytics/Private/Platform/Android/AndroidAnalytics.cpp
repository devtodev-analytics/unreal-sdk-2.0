...
void AndroidAnalytics::GetGender(const FDTDGetterGenderDelegate *onResult)
{
	...
	jclass jclsGetterLongCallback = FAndroidApplication::FindJavaClass(TCHAR_TO_ANSI(*(enganePath + "/GameActivity$DevToDevGetterLongCallback")));
	jmethodID jmidGetterLongCallbackInit = Env->GetMethodID(jclsGetterLongCallback, "<init>", TCHAR_TO_ANSI(*("(L" + enganePath + "/GameActivity;Ljava/lang/Object;)V")));
...
void AndroidAnalytics::UpdateLocalConfig()
{
	...
	if (FJsonSerializer::Deserialize(JsonReader, JsonParsed))
	{
		for (auto currJsonValue = JsonParsed->Values.CreateConstIterator(); currJsonValue; ++currJsonValue)
		{
			...
			FString strValue;
			if (jsonObject->TryGetStringField("value", strValue)){
				configValue.StringValue = strValue;
			}

			int32 int32Value;
			if (jsonObject->TryGetNumberField("value", int32Value)){
				configValue.IntegerValue = int32Value;
			}

			int64 longValue;
			if (jsonObject->TryGetNumberField("value", longValue)){
				configValue.LongValue = longValue;
			}

			double floatValue;
			if (jsonObject->TryGetNumberField("value", floatValue)){
				configValue.FloatValue = (float)floatValue;
			}

			bool boolValue;
			if (jsonObject->TryGetBoolField("value", boolValue)){
