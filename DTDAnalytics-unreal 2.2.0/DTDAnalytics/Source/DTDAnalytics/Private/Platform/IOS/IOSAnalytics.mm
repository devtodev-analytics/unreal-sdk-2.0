...
void IOSAnalytics::SetInitializationCompleteCallback(const FDTDAnalyticsInitializationCompleteDelegate* callback)
{
...
void IOSAnalytics::SetTester(bool tester)
{
	[::DTDUserCard setTester:IOSConverter::ConvertToObjC(tester)];
}
...
void IOSAnalytics::InitializeWithAbTest(const FString& appKey,
...
void IOSAnalytics::InitializeWithConfigWithAbTest(const FString& appKey,
...
float IOSAnalytics::GetRemoteConfigWaiting()
{
	return [::DTDRemoteConfig remoteConfigWaiting];
}

void IOSAnalytics::SetRemoteConfigWaiting(float value)
{
	[::DTDRemoteConfig setRemoteConfigWaiting:(double)value];
}

float IOSAnalytics::GetGroupDefinitionWaiting()
{
	return [::DTDRemoteConfig groupDefinitionWaiting];
}

void IOSAnalytics::SetGroupDefinitionWaiting(float value)
{
	[::DTDRemoteConfig setGroupDefinitionWaiting:(double)value];
}

void IOSAnalytics::SetDefaults(const FDTDRemoteConfigDefaults& defaults)
{
...
void IOSAnalytics::UpdateLocalConfig()
{
	localConfig.Reset();
    @autoreleasepool {
        ...
        for (NSString* key in [rawConfig allKeys]) {
            ...
            value.Source = IOSConverter::Convert([nativeValue source]);
            value.StringValue = IOSConverter::Convert([nativeValue stringValue]);
            value.FloatValue = [nativeValue floatValue];
            value.IntegerValue = IOSConverter::ConvertToInt32([nativeValue integerValue]);
            value.LongValue = IOSConverter::ConvertToInt64([nativeValue integerValue]);
            value.BoolValue = IOSConverter::ConvertToCpp([nativeValue boolValue]);
            localConfig.Add(IOSConverter::Convert(key), value);
        }
    }
}

void IOSAnalytics::ApplyConfig()
{
	[::DTDRemoteConfig applyConfig];
	UpdateLocalConfig();
}

void IOSAnalytics::ResetConfig()
{
	[::DTDRemoteConfig resetConfig];
	UpdateLocalConfig();
}

TMap<FString, FDTDRemoteConfigValue> IOSAnalytics::GetConfig() {
    return localConfig;
}

FDTDRemoteConfigValue IOSAnalytics::GetRemoteConfigValue(const FString& key) {
...
bool IOSAnalytics::HasKey(const FString& key) {
	return localConfig.Contains(key);
}

void IOSAnalytics::CacheTestExperiment() {
